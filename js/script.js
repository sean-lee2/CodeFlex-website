...
class TwinWorksApp {
    ...
    startSimulation() {
        ...
        this.simulationData.status = '실행 중';
        ...
        const simulationInterval = setInterval(() => {
            ...
            this.simulationData.production += Math.floor(Math.random() * 5) + 1;
            this.simulationData.efficiency = Math.min(100, this.simulationData.efficiency + Math.random() * 2);
            this.simulationData.temperature = 25 + Math.random() * 10;
    ...
    pauseSimulation() {
        this.isSimulationRunning = false;
        this.simulationData.status = '일시정지';
        this.updateSimulationDisplay();
    }
    ...
    updateSimulationDisplay() {
        const elements = {
            'production-count': this.simulationData.production,
            'efficiency': Math.floor(this.simulationData.efficiency) + '%',
            'temperature': this.simulationData.temperature.toFixed(1) + '°C',
            'status': this.simulationData.status
        };
    ...
    // 3D 모델 애니메이션
    animate3D(type) {
        const model = document.querySelector('.3d-model > div');
        if (!model) return;

        model.style.transition = 'transform 1s ease';
        
        switch (type) {
            case 'rotate':
                model.style.transform = 'rotateY(360deg) rotateX(15deg)';
                setTimeout(() => {
